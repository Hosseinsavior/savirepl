...
run_xray() { 
   re_uuid=$(curl -s $REPLIT_DB_URL/re_uuid)
    ...
    echo "tls：开启"
    echo 
replit_xray_trojan="trojan://${user_uuid}@${REPL_SLUG}.${REPL_OWNER}.repl.co:443?security=tls&type=ws&host=${REPL_SLUG}.${REPL_OWNER}.repl.co&path=/$user_uuid#replit_xray_trojan"
yellow "分享链接如下"    
